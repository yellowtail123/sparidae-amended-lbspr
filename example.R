...
#   protogyny  -> SPR_fem is the status; SPR_male / SPR_fem shows the male arm is the depleted one
# =====================================================================
...
rows <- lapply(seq_len(nrow(lh)), function(i) {
  ...
  data.frame(
    ...
    SPR_bind = round(sx$SPR_bind, 3),         # precautionary minimum, retained for audit
    deficit  = round(sx$SPR_male / sx$SPR_fem, 3),   # male arm relative to the egg arm; 1 = parity
    shift    = round(sx$SPR_fem - sx$SPR_gono_check, 3),
    stringsAsFactors = FALSE)
})
...
cat("(SPR_std = standard LBSPR; SPR_fem = the status the amendment reports;",
    "deficit = SPR_male / SPR_fem, read against 1)\n\n")
print(out, row.names = FALSE)
cat("\nNote: gonochore SPR_fem equals SPR_std (the amendment is a generalisation, not a",
    "\ncompeting method); for protogyny SPR_male falls well below SPR_fem, which is the deficit\n",
    "the diagnostic is there to show.\n")
...
#
#   res <- fit_lbspr_one(L, lh_row)   # res$SPR is standard LBSPR; res$SPR_fem is the amended status
#
